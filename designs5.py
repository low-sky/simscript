...
values = np.loadtxt('Design5.csv',skiprows=1,delimiter=',')

rootdir = os.path.expanduser("~")+'/SimSuite5/'
rootname = 'Design'
...
for idx,bval in enumerate(bvals):
    ...
    with open(dirname+'DrivenTurbulenceCTMHD','a') as template:
        ...
        template.close()
    print(dirname)
params.write('d5_parameter_table.ascii',format='ascii.fixed_width')
params.write('d5_parameter_table.csv',format='ascii',delimiter=',')
...
if Fiducials:
    ...
    tcross = (BoxSize/(MachVals*SoundSpeed)).to(u.s)
    Bvals = ((8*np.pi*density*SoundSpeed**2/betavals)**(0.5)).value*(u.G)
    kminVals = 4*np.ones(len(betavals))
    ...
    for idx,bval in enumerate(bvals):
        ...
        if not os.path.isdir(dirname):
            ...
            with open(dirname+'DrivenTurbulenceCTMHD','a') as template:
                ...
                template.close()
                print(dirname)
    fparams.write('d5_fiducial_table.ascii',format='ascii.fixed_width')
    fparams.write('d5_fiducial_table.csv',format='ascii',delimiter=',')
