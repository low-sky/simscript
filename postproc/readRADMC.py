...
def readimage(filename=None, imagefile=None ):
	...
	lineTest = lines[(int(nf)+4):-1]
	for i in range(len(lines[(int(nf)+4):-1])):
		lineTest[i] = lines[(int(nf)+4)+i].replace('\n','')

	print(len(lineTest))
	lineTest = filter(lambda item: item.strip(), lineTest)
	print(len(lineTest))
	print(len(temp))
	for i in range(len(lineTest)):
		temp[i] = float(lineTest[i])
	print('Worked')
